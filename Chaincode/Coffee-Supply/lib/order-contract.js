...
const CoffeeContract = require('./coffee-contract');
...
class OrderContract extends Contract {
    ...
    async createOrder(ctx, orderId) {
        ...
        !transientData.has('ImporterName')
            ) {
                throw new Error(
            ...
            orderAsset.ImporterName = transientData.get('ImporterName').toString();
    ...
    async deleteOrder(ctx, orderId) {
        const mspID = ctx.clientIdentity.getMSPID();
        if (mspID === 'importer-coffee-com' || mspID === 'exporter-coffee-com') {
            const exists = await this.orderExists(ctx, orderId);
            if (!exists) {
                throw new Error(`The asset order ${orderId} does not exist`);
            }
